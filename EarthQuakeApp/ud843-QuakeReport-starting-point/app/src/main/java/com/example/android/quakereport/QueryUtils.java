...
public final class QueryUtils {
    ...
    public static ArrayList<EarthQuakeBean> extractEarthquakes(String OUTPUT_JSON) {
        ...
        try {
            ...
            EarthQuakeBean earthQuakeBeanItem;
            for(int i = 0 ; i< earthQuakeBean.getFeatures().size();i++){
                earthQuakeBeanItem = new EarthQuakeBean();
                earthQuakeBeanItem.setMagnitude(Double.parseDouble(earthQuakeBean.getFeatures().get(i).getProperties().getMag().toString()));
                earthQuakeBeanItem.setLocation(earthQuakeBean.getFeatures().get(i).getProperties().getPlace());
                earthQuakeBeanItem.setDate(formatDate(earthQuakeBean.getFeatures().get(i).getProperties().getTime()));
                earthQuakeBeanItem.setTime(formatTime(earthQuakeBean.getFeatures().get(i).getProperties().getTime()));
                earthquakes.add(earthQuakeBeanItem);
            }
    ...
    public static ArrayList<EarthQuakeBean> getEarthQuakeData(String urlToGetData){
        ArrayList<EarthQuakeBean> earthQuakeBeans = new ArrayList<>();
        try {
            URL url = new URL(urlToGetData);
            earthQuakeBeans= makeHTTPRequest(url);
    ...
    private static ArrayList<EarthQuakeBean> makeHTTPRequest(URL url) throws IOException{
        String response="";
        ...
        if(response!=null && !response.equals("")){
            earthQuakeBeans = extractEarthquakes(response);
        }
        return earthQuakeBeans;
